...
function banner(){
		...
		echo -e "\t${p}   Author${m}: ${p}Trik Dan Tutorial"
		echo -e "\t${p}    Youtube Trik Dan Tutorial"
		echo -e "\t${p}    Share Like & Subscribe!!!!"
		echo -e "\t${p}_________________________"
...
function sec(){
		printf "\t\b${p} [${h}•${p}] Mohon Tunggu${m}:    ${p}"
			for w in {3..0};do
			get=$(printf "\b${w}")
...
read proxy;
	...
	json=$(curl \
	-s \
	"https://api.shodan.io/shodan/host/{${proxy}}?key={OZi7Lq6PPz8B15jO1nFa1Hagt5lEBI}")
